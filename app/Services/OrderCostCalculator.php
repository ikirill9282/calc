...
class OrderCostCalculator
{
    ...
    public function calculate(): array
    {
        ...
        return [
            ...
            'total' => (int) ceil(($pick ?? 0) + ($delivery ?? 0) + ($additional ?? 0)),
        ];
    }
    ...
    protected function calculatePickAmount(): ?int
    {
        ...
        if ($this->canCalculateBoxes()) {
            ...
            $costVolume = $volume * $tariffs['pick_tariff_vol'];
            $result += max($min, $costVolume);
        }

        if ($this->canCalculatePallets()) {
            $costPallet = $this->normalizeFloat($this->order->pallets_count) * $tariffs['pick_tariff_pallete'];

            return (int) ceil($costPallet);
        }
    ...
    protected function calculateDeliveryAmount(): ?float
    {
        ...
        if ($this->canCalculateBoxes()) {
            $volume = $this->normalizeFloat($this->order->boxes_volume);
            $volume = $this->roundVolumeStep($volume);
            $costVolume = $volume * $tariffs['delivery_tariff_vol'];
            $result += max($tariffs['delivery_tariff_min'], $costVolume);
        }

        if ($this->canCalculatePallets()) {
            $costPallet = $this->normalizeFloat($this->order->pallets_count) * $tariffs['delivery_tariff_pallete'];
