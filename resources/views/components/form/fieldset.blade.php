...
<x-card class="relative !p-4 !pt-12 sm:!p-6 sm:!pt-12 md:!p-12 group/card {{ $set_class }}">
  <div class="absolute hidden z-20 rounded-xl top-0 left-0 w-full h-full group-[.disabled]/card:block bg-primary-50/80 dark:bg-primary-900/80">
  </div>
  @if($set_title || $set_description)
    <div class="rounded-full z-30 flex border-0 absolute top-0 text-nowrap left-4 sm:left-8 md:left-16 translate-y-[-50%] shadow 
  ...
  @if($set_loading)
    <div wire:loading class="absolute rounded-xl top-0 left-0 w-full h-full bg-primary-50/80 dark:bg-primary-900/80 z-20">
