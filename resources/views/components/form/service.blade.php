...
<div class="flex flex-col sm:flex-row sm:gap-4 distributor-group">
    ...
    <div class="flex flex-wrap justify-start items-start gap-3">
        @foreach (\App\Models\Distributor::all() as $distributor)
            <div wire:click="setField('{{ $name }}', {{ $distributor->id }})" class="group flex distributor-item">
                <input type="radio" name="{{ $name }}" value="{{ $distributor->id }}"
                    class="w-0 h-0 leading-0 opacity-0"
                    {{ $this->getField($name) == $distributor->id ? 'checked' : '' }}>
                  ...
                  <div class="flex justify-center items-center border rounded-xl hover:cursor-pointer transition
                    ...
                    @if ($distributor->logo)
                        <span class="p-2 pe-0">@include("icons.$distributor->logo", ['width' => 38, 'height' => 38])</span>
                    @endif
                    <div class="px-4 py-2">
                        {{ $distributor->title }}
                    </div>
